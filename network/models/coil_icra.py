...
class CoILICRA(nn.Module):
    ...
    def __init__(self, params):
        ...
        # change the name of branch when adding extra layers to prevent key error while loading checkpoint
    ...
    # this portion is required for grad cam
    def perception_hook(self, grad):
        self.gradients = grad

    def get_perception_activations(self, x):
        return self.get_perception_layers(x)

    def get_perception_gradients(self):
        return self.gradients
    # grad cam portion ends here
    ...
    def forward(self, x, a):
        ...
        # register the hook for the specified conv block (hardcoded) when running gradcam
        h = inter[4].register_hook(self.perception_hook)
    ...
    def get_prefinal_layer(self, x, a, branch_number):
        # get second last layer activations to store them

        x, inter = self.perception(x)
        m = self.measurements(a)
        j = self.join(x, m)
        speed_prefinal_output = self.speed_branch(x)
