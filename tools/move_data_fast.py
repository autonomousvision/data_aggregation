"""
Script to move data fast using multiprocessing and shutil

Usage: python move_data_fast.py <source_dir> <target_dir> <start_episode_num> <end_episode_num>
"""
...
def copy_episodes(episode):
	...
	for file_name in episode_data:
		...
		if 'metadata' in file_name:
			shutil.move(os.path.join(source_dir, episode, file_name), os.path.join(target_dir, episode, file_name))
		...
		if 'measurements_' in file_name:
			...
			shutil.move(central_image, os.path.join(target_dir, episode, 'CentralRGB_%s.png'%episode_number))
			shutil.move(left_image, os.path.join(target_dir, episode, 'LeftRGB_%s.png'%episode_number))
			shutil.move(right_image, os.path.join(target_dir, episode, 'RightRGB_%s.png'%episode_number))
