...
class CityTrack(object):
    ...
    def is_far_away_from_route_intersection(self, current_node):
        ...
        if self._route is None:
            ...
            #     log.write('Impossible to find route'
    ...
    def compute_route(self, node_source, source_ori, node_target, target_ori):
        ...
        # np.set_printoptions(edgeitems=3, infstr='inf', threshold=np.nan, linewidth=129) # original
        np.set_printoptions(edgeitems=3, infstr='inf', threshold=sys.maxsize, linewidth=129) # changed by aditya
        ...
        if route is None:
            # with open('/is/sg2/aprakash/Documents/debug', 'a+') as log:
            #     log.write('Impossible to find route, returning previous route \n')
            #     log.close()
