...
class Recording(object):
    ...
    def __init__(self,
        ...
        save_path = os.path.join('/is/rg/avg/aprakash','_benchmarks_results')
        if not os.path.exists(save_path):
            os.mkdir(save_path)

        # Generate the full path for the log files
        self._path = os.path.join(save_path, name_to_save)
    ...
    def write_summary_results(self, experiment, pose, rep,
        ...
        if corruption_number is not None and severity is not None:
            self._dict_summary['corruption_number'] = corruption_number
            self._dict_summary['severity'] = severity
        '''
