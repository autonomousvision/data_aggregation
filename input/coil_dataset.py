...
class CoILDataset(Dataset):
    ...
    def __init__(self, root_dir, transform=None, preload_name=None):
        ...
        # load all the textures (all the texture images names need to be stored in a separate file first (format same as preload files))
        # self.texture_names = np.load(os.path.join('_preloads', 'texture_names.npy'))
    ...
    def _pre_load_image_folders(self, path):
        ...
        for episode in episodes_list:
            ...
            if number_of_hours_pre_loaded > 3*g_conf.NUMBER_OF_HOURS: # added 3 for left, central and right
            ...
            for measurement in measurements_list[:-3]:
                ...
                if self.is_measurement_partof_experiment(final_measurement):
                    ...
                    # seg = 'CentralSemanticSeg_' + data_point_number + '.png'
                    sensor_data_names.append(os.path.join(episode.split('/')[-1], rgb))
                    # semantic_segmentation_maps.append(os.path.join(episode.split('/')[-1], seg))
                ...
                if self.is_measurement_partof_experiment(final_measurement):
                    ...
                    # seg = 'LeftSemanticSeg_' + data_point_number + '.png' 
                    sensor_data_names.append(os.path.join(episode.split('/')[-1], rgb))
                    # semantic_segmentation_maps.append(os.path.join(episode.split('/')[-1], seg))
                    count_added_measurements += 1
                ...
                if self.is_measurement_partof_experiment(final_measurement):
                    ...
                    # seg = 'RightSemanticSeg_' + data_point_number + '.png'
                    sensor_data_names.append(os.path.join(episode.split('/')[-1], rgb))
                    # semantic_segmentation_maps.append(os.path.join(episode.split('/')[-1], seg))
                    count_added_measurements += 1
        ...
        if self.preload_name is not None:
            ...
            # np.save(os.path.join('_preloads', self.preload_name), [sensor_data_names, float_dicts, semantic_segmentation_maps])

        return sensor_data_names, float_dicts
        # return sensor_data_names, float_dicts, semantic_segmentation_maps
