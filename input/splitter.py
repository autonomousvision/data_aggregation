...
from .coil_sampler import PreSplittedSampler, RandomSampler, UniformDaggerSampler, FixedSampler
...
def select_balancing_strategy(dataset, iteration, number_of_workers):
    ...
    elif g_conf.SAMPLE == 'Dagger':
        sampler = UniformDaggerSampler(keys, iteration * g_conf.BATCH_SIZE)
    elif g_conf.SAMPLE == 'Fixed':
        sampler = FixedSampler(keys, iteration * g_conf.BATCH_SIZE)
    else:
        sampler = RandomSampler(keys, iteration * g_conf.BATCH_SIZE) # default sampler
