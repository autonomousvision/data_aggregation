...
class UniformDaggerSampler(Sampler):
    ...
    def __init__(self, keys, executed_iterations):
        ...
        self.keys = keys
        self.old_keys = self.keys[:1009470] # hard coded for now
        self.dagger_keys = self.keys[1009470:]

    def __iter__(self):
        # print (self.old_keys[-1], self.dagger_keys[-1])
        sampled_indices = []
        for _ in range(self.iterations_to_execute):
            # coin = random.choice([0, 1])
            coin = random.uniform(0,1) # vary dagger proportion here
            curr_index = random.choice(self.old_keys) if coin<0.65 else random.choice(self.dagger_keys)
            sampled_indices.append(curr_index)
...
class FixedSampler(Sampler):
    r"""Samples elements in the iterative sequence in a given list
    ...
    def __iter__(self):
        return iter([self.iterations_to_execute%len(self.keys) for _ in range(self.iterations_to_execute)])
