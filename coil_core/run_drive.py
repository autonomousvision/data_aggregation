...
def driving_benchmark(checkpoint_number, gpu, town_name, experiment_set, exp_batch, exp_alias,
    ...
    try:
        ...
        '''
        # changing the path below, original: _logs, exp_batch, exp_alias
        path_to_save = exp_batch+'_'+exp_alias+'_'+str(checkpoint_number)+'_drive_'+control_filename+'_'+'_'.join(g_conf.PROCESS_NAME.split('_')[1:])
        # print ('path_to_save: ', path_to_save)
        dir_path = os.path.join('/is/rg/avg/aprakash/_benchmark_results', path_to_save,
                                 g_conf.PROCESS_NAME + '_csv')
        if not os.path.exists(dir_path):
            os.mkdir(dir_path)
        file_base = os.path.join('/is/rg/avg/aprakash/_benchmark_results', path_to_save,
        ...
        for i in range(len(task_list)):
            # write_header_control_summary(file_base, task_list[i]) # changed here by aditya
...
def execute(gpu, exp_batch, exp_alias, drive_conditions, params):
    ...
    try:
        ...
        if latest is None:  # When nothing was tested, get latest returns none, we fix that.
            ...
            '''
            # changing the path below, original: _logs, exp_batch, exp_alias
            path_to_save = exp_batch+'_'+exp_alias+'_'+str(latest)+'_drive_'+control_filename+'_'+'_'.join(g_conf.PROCESS_NAME.split('_')[1:])
            print ('path_to_save: ', path_to_save)
            file_base = os.path.join('/is/rg/avg/aprakash/_benchmark_results', path_to_save,
                                     g_conf.PROCESS_NAME + '_csv', control_filename)
            '''
