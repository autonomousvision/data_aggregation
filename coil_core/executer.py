...
def execute_train(gpu, exp_batch, exp_alias, suppress_output=True, number_of_workers=12):
    """
    Args:
        gpu: gpu being used for this execution.
        exp_batch: folder this driving experiment is being executed
        exp_alias: experiment alias, file name, to be executed.
...
def execute_drive(gpu, exp_batch, exp_alias, exp_set_name, params):
    ...
    Args:
        ...
        exp_batch: folder this driving experiment is being executed
        exp_alias: experiment alias, file name, to be executed.
        exp_set_name: driving environment
    ...
    p = multiprocessing.Process(target=run_drive.execute,
                               args=(gpu, exp_batch, exp_alias, exp_set_name, params))
    p.start()


# assigning gpus for multi-gpu training
def assign_gpus(free_gpus, resources_on_most_free_gpu, cost):
    gpu_ids = []
    while resources_on_most_free_gpu >= cost:
        free_gpus, resources_on_most_free_gpu, gpu_number = allocate_gpu_resources(
            free_gpus, cost)
        if gpu_number not in gpu_ids:
            gpu_ids.append(gpu_number)

    return free_gpus, resources_on_most_free_gpu, gpu_ids
    ...
    while True:
        ...
        while resources_on_most_free_gpu >= min([allocation_parameters['train_cost'],
            ...
            if process_specs['type'] == 'train' and resources_on_most_free_gpu >= \
                ...
                free_gpus, resources_on_most_free_gpu, gpu_ids = assign_gpus(free_gpus, 
                                resources_on_most_free_gpu, allocation_parameters['train_cost'])
                execute_train(gpu_ids, process_specs['folder'], process_specs['experiment'],
                              params['number_of_workers'])
                process_specs.update({'gpu': gpu_ids})
            ...
            # ADD DRIVE TO EXECUTE
                    ...
                    and (train_status == 'Iterating' or train_status == 'Loading' or
                         train_status == 'Finished' or train_status == 'Error'):
                free_gpus, resources_on_most_free_gpu, gpu_number = allocate_gpu_resources(free_gpus,
                                allocation_parameters['drive_cost'])
            ...
            # ADD VALIDATION TO EXECUTE
                    ...
                    and (train_status == 'Iterating' or train_status == 'Loading' or
                         train_status == 'Finished' or train_status == 'Error'):
                free_gpus, resources_on_most_free_gpu, gpu_ids = assign_gpus(free_gpus, 
                                resources_on_most_free_gpu, allocation_parameters['validation_cost'])
                execute_validation(gpu_ids, process_specs['folder'], process_specs['experiment'],
